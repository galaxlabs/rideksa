...
class FrappeApiClient {
  ...
  Future<void> login({required String email, required String password}) async {
    final bodyStr = 'usr=${Uri.encodeComponent(email)}&pwd=${Uri.encodeComponent(password)}';
    ...
    _cookie = setCookie.split(';').firstWhere((p) => p.trim().startsWith('sid=')).trim();
  ...
  Future<void> loginWithFirebaseIdToken(String idToken) async {
    ...
    if (response.statusCode >= 400) {
      throw ApiException(decoded['message']?.toString() ?? 'Frappe Firebase login failed');
    }
    final setCookie = response.headers['set-cookie'];
    if (setCookie != null && setCookie.contains('sid=')) {
      _cookie = setCookie.split(';').firstWhere((p) => p.trim().startsWith('sid=')).trim();
    ...
    if (message is Map && message['api_token'] != null) {
      await _storage.write(key: 'frappe_api_token', value: message['api_token'].toString());
    }
  }

  Future<void> logout() async {
    try { await callMethod('logout'); } catch (_) {}
    _cookie = null;
    await _storage.delete(key: 'frappe_session');
  }

  Future<dynamic> callMethod(String method, {Map<String, dynamic>? query, Map<String, dynamic>? body, bool requiresSession = true}) async {
    ...
    final decoded = _decode(response);
    if (response.statusCode >= 400) {
      throw ApiException(decoded['_server_messages']?.toString() ?? 'HTTP ${response.statusCode}');
    }
    return decoded;
  }

  Future<List<dynamic>> getList(String doctype, {Map<String, dynamic>? filters, List<String>? fields, int limit = 50}) async {
    await _restoreSession();
    final params = <String, dynamic>{
      'fields': jsonEncode(fields ?? ['*']), 'limit_page_length': limit,
    };
    ...
    if (response.statusCode >= 400) throw ApiException(data['message'] ?? 'HTTP ${response.statusCode}');
  ...
  Future<Map<String, dynamic>> createDoc(String doctype, Map<String, dynamic> doc) async {
    await _restoreSession();
    final uri = _resourceUri(doctype);
    final headers = <String, String>{'Content-Type': 'application/json', 'Accept': 'application/json'};
    if (_cookie != null) headers['Cookie'] = _cookie!;
    final response = await _client.post(uri, headers: headers, body: jsonEncode(doc));
    final data = _decode(response);
    if (response.statusCode >= 400) throw ApiException(data['message'] ?? 'HTTP ${response.statusCode}');
    return data['data'] as Map<String, dynamic>;
  }
  ...
  Future<List<Map<String, dynamic>>> getAvailableRoutes() async {
    final result = await callMethod('$_apiBase.route.list_routes', requiresSession: false);
    return _messageList(result);
  }

  Future<Map<String, dynamic>> createBooking({
    required String pickup, required String dropoff, required String date,
    String? customerName, String? phone, String? route, int passengers = 1,
    double fare = 0, String? vehicleType, double? pickupLat, double? pickupLng,
    List<Map<String, dynamic>>? passengerList,
  }) async {
    final body = <String, dynamic>{
      'pickup_point': pickup, 'drop_point': dropoff, 'booking_date': date,
      'passenger_count': passengers, 'fare_amount': fare,
    };
  ...
  Future<Map<String, dynamic>> completeOnboarding({
    ...
    if (purpose == 'passenger') {
      return _messageMap(await callMethod(
        '$_apiBase.onboarding.create_passenger_profile',
        body: {
          'full_name': fullName, 'mobile_no': phone, 'nationality': nationality,
          'id_document_type': idDocumentType, 'id_number': idNumber,
    ...
    if (purpose == 'captain') {
      return _messageMap(await callMethod(
        ...
        body: {
          'full_name': fullName, 'mobile_no': phone, 'nationality': nationality,
          'id_document_type': idDocumentType, 'national_id': idNumber,
          'license_no': licenseNo, 'license_expiry_date': licenseExpiryDate,
          'iqama_no': iqamaNo, 'iqama_expiry_date': iqamaExpiryDate,
          'driver_card_no': driverCardNo, 'driver_card_expiry_date': driverCardExpiryDate,
    ...
    if (purpose == 'customer_company') {
      return _messageMap(await callMethod(
        ...
        body: {
          ...
          'vat_no': vatNo, 'tax_id': taxId, 'cr_no': crNo,
          'phone': phone, 'email': null, 'address': address,
          'city': city, 'country': country, 'full_name': fullName,
          'mobile_no': phone,
        },
      ));
    }
    final result = await callMethod(
      ...
      body: {
        ...
        'legal_name': legalName, 'company_name_ar': companyNameAr,
        'vat_no': vatNo, 'tax_id': taxId, 'cr_no': crNo, 'license_no': licenseNo,
        'phone': phone, 'address': address, 'city': city, 'country': country,
        'mobile_no': phone, 'service_types': serviceTypes,
        'full_name': fullName,
      },
    );
  ...
  Future<Map<String, dynamic>> registerVehicle({
    ...
    required String vehicleMake, required String vehicleModel,
    String? vehicleType, int? modelYear, String? color,
    int? passengerCapacity, String? registrationNo,
  }) async {
    return _messageMap(await callMethod(
      ...
      body: {
        ...
        'vehicle_type': vehicleType, 'model_year': modelYear,
        'color': color, 'passenger_capacity': passengerCapacity,
  ...
  Future<Map<String, dynamic>> getVehicleCatalog({String? make, String? type}) async {
  ...
  Future<Map<String, dynamic>> inviteCompanyMember({
    required String company, required String email, required String role,
  }) async {
    return _messageMap(await callMethod(
  ...
  Future<Map<String, dynamic>> verifyPlayIntegrity(String integrityToken) async {
    return _messageMap(await callMethod(
  ...
  Future<List<Map<String, dynamic>>> getMyNotifications() async {
    final result = await callMethod('$_apiBase.notifications.list_my_notifications');
    return _messageList(result);
  }
  ...
  Future<Map<String, dynamic>> createGroupInvite(String bookingName, {int? expiresInHours}) async {
    return _messageMap(await callMethod(
  ...
  dynamic _decode(http.Response response) {
    try { return jsonDecode(response.body); } catch (_) { return {'message': response.body}; }
  }
