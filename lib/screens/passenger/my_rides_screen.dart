...
class _MyRidesScreenState extends State<MyRidesScreen> {
  ...
  bool _canCancel(Map<String, dynamic> ride) {
    ...
    if (negotiation == 'Trip Created' || negotiation == 'Confirmed') {
      return false;
    }
    return true;
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final startedTripIds = trips
          .where((trip) => _isStartedTrip(trip['trip_status']?.toString()))
          .map((trip) => trip['name']?.toString())
          .whereType<String>()
          .toSet();
      final currentBookings = bookings.where((booking) {
        final trip = booking['trip']?.toString();
        return trip == null || trip.isEmpty || !startedTripIds.contains(trip);
      }).toList();
      if (!mounted) return;
      setState(() {
        _rides = currentBookings;
        _loading = false;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  bool _isStartedTrip(String? status) {
  ...
  Future<void> _cancel(Map<String, dynamic> ride) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Your ride from ${ride['pickup_point'] ?? ride['route'] ?? ''} will be cancelled.',
        ),
        actions: [
          TextButton(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: InkWell(
                      onTap: () => context.push(
                        '/passenger/ride-detail/${Uri.encodeComponent(name.toString())}',
                      ),
                      child: Padding(
                        padding: const EdgeInsets.all(16),
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 6),
                                Text(
                                  '${ride['passenger_count'] ?? ride['seat_count'] ?? 1}',
                                  style: TextStyle(
                                    color: Colors.grey[600],
                                    fontSize: 13,
                                  ),
