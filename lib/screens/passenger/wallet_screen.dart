        ...
        child: Column(children: [
          Container(
            ...
            child: Column(children: [
              ...
              Text(wallet.balanceFormatted, style: const TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.bold)),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('Total: ${wallet.transactions.length}', style: TextStyle(color: AppColors.textSecondary)),
          ]),
          ...
          if (wallet.transactions.isEmpty)
            Card(child: Padding(padding: const EdgeInsets.all(32), child: Center(child: Column(children: [
              ...
              const SizedBox(height: 8),
              Text('No transactions yet', style: TextStyle(color: AppColors.textSecondary.withAlpha(150))),
            ]))))
          else
            ...wallet.transactions.map((tx) => Card(
              child: ListTile(
                leading: Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(
                    color: tx.type.name == 'credit' ? AppColors.success.withAlpha(20) : Colors.red.withAlpha(20),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Icon(
                    tx.type.name == 'credit' ? Icons.arrow_downward : Icons.arrow_upward,
                    color: tx.type.name == 'credit' ? AppColors.success : Colors.red,
                    size: 20,
                  ),
                ),
                title: Text(tx.description?.split('\n').first ?? tx.reason.name, style: const TextStyle(fontWeight: FontWeight.w500)),
                subtitle: Text('${tx.createdAt.toString().split(' ').first} • ${tx.status.name}'),
                trailing: Text(tx.amountFormatted, style: TextStyle(
        ...
        content: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                DropdownMenuItem(value: 'test_wallet', child: Text('Testing Credit')),
                ...
                DropdownMenuItem(value: 'bank_transfer', child: Text('Saudi Bank Transfer')),
              ],
            ...
            if (paymentMethod == 'bank_transfer') ...[
              ...
              OutlinedButton.icon(
                onPressed: () async {
                  const target = 'android.settings.ACTION_NOTIFICATION_LISTENER_SETTINGS';
                  final canOpen = await canLaunchUrl(Uri(scheme: 'android.settings', host: target));
                  if (canOpen) {
                    await launchUrl(Uri(scheme: 'android.settings', host: target), mode: LaunchMode.externalApplication);
                  }
                },
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (amt > 0) {
                context.read<WalletProvider>().topUp(amt, paymentMethod: paymentMethod);
                Navigator.pop(ctx);
              }
            },
