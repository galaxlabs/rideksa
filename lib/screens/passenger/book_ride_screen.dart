...
class _BookRideScreenState extends State<BookRideScreen> {
  ...
  void initState() {
    ...
    if (route != null) {
      ...
      final to = route['to'] as String? ?? route['destination'] as String? ?? '';
      _pickupController.text = from;
      _dropoffController.text = to;
      _routeKm = (route['km'] as num?)?.toDouble() ?? (route['distance_km'] as num?)?.toDouble() ?? 0;
    }
  ...
  double get _flatEstimate => _routeKm > 0
      ? _baseFare > 0 ? _baseFare : (_routeKm * 1.5)
  ...
  Future<void> _refreshFare() async {
    ...
    final display = _fareMode == 'flat'
        ...
        : (_perPassengerRate > 0 ? _perPassengerRate * _passengers : _flatEstimate * _passengers);
  ...
  Future<void> _pickDate() async {
    ...
    if (d != null) setState(() { _date = d; _dateController.text = _formatDate(d); });
  }
  ...
  List<Map<String, dynamic>> _buildPassengerList() {
    ...
    return [
      {
        'passenger_name': context.read<AuthProvider>().user?.displayName ?? 'Passenger',
    ...
    try {
      ...
      if (_bookingMode == 'now') {
        ...
        if (!ok || loc.position == null) {
          throw Exception('Current booking requires GPS pickup from your current city/location. Use scheduled booking for other cities.');
        }
        if (_pickupPlace == null) _pickupController.text = 'Current GPS location';
      }
      final passengerRows = _buildPassengerList();
      final rowCount = passengerRows.length > 1 ? passengerRows.length : _passengers;
      await rideProvider.createRideRequest(
        ...
        contractDuration: _serviceType == 'pick_drop' ? _durationController.text.trim() : null,
        pickupTime: _serviceType == 'pick_drop' ? _pickupTimeController.text.trim() : null,
        dropoffTime: _serviceType == 'pick_drop' ? _dropoffTimeController.text.trim() : null,
        rentalDays: _serviceType == 'rent_car' ? int.tryParse(_rentalDaysController.text.trim()) ?? 1 : 1,
        groupName: _isGroupBooking ? _groupController.text.trim() : null,
        passengerNames: passengerRows.map((r) => r['passenger_name'].toString()).toList(),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ride request submitted!'), backgroundColor: AppColors.success));
  ...
  void dispose() {
    ...
    _dateController.dispose(); _notesController.dispose(); _priceController.dispose();
    _groupController.dispose();
    _pickupTimeController.dispose(); _dropoffTimeController.dispose();
    _durationController.dispose(); _rentalDaysController.dispose();
    for (final row in _passengerRows) { row.controller.dispose(); }
    _places.dispose();
    super.dispose();
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          DropdownButtonFormField<String>(
            value: _serviceType,
            decoration: const InputDecoration(labelText: 'Service Type', prefixIcon: Icon(Icons.miscellaneous_services)),
            items: const [
              DropdownMenuItem(value: 'ride', child: Text('Ride / Trip')),
              DropdownMenuItem(value: 'pick_drop', child: Text('Scheduled Pick & Drop')),
              DropdownMenuItem(value: 'rent_car', child: Text('Rent a Car / Vehicle')),
            ],
          ...
          if (_serviceType == 'pick_drop') ...[
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(labelText: 'Routine Type', prefixIcon: Icon(Icons.business_center)),
              items: const [
                DropdownMenuItem(value: 'kids_school', child: Text('Kids / School')),
                DropdownMenuItem(value: 'factory', child: Text('Factory Staff')),
                DropdownMenuItem(value: 'site', child: Text('Project / Site')),
            ...
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: TextField(controller: _pickupTimeController, decoration: const InputDecoration(labelText: 'Pickup Time', prefixIcon: Icon(Icons.schedule)))),
              const SizedBox(width: 10),
              Expanded(child: TextField(controller: _dropoffTimeController, decoration: const InputDecoration(labelText: 'Drop Time', prefixIcon: Icon(Icons.schedule_send)))),
            ]),
            const SizedBox(height: 12),
            TextField(controller: _durationController, decoration: const InputDecoration(labelText: 'Contract Duration', hintText: 'Example: 1 month, 6 months, school term', prefixIcon: Icon(Icons.date_range))),
            const SizedBox(height: 12),
          ],
          if (_serviceType == 'rent_car') ...[
            TextField(controller: _rentalDaysController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Rental Days', prefixIcon: Icon(Icons.car_rental))),
            const SizedBox(height: 12),
          ],
          SegmentedButton<String>(
            segments: const [
              ButtonSegment(value: 'now', label: Text('On time'), icon: Icon(Icons.flash_on)),
              ButtonSegment(value: 'scheduled', label: Text('Scheduled'), icon: Icon(Icons.event)),
            ],
            selected: {_bookingMode},
            onSelectionChanged: (v) => setState(() => _bookingMode = v.first),
          ),
          const SizedBox(height: 8),
          Text(_bookingMode == 'now'
          ...
          Text('Vehicle Type', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 8, children: ['Sedan', 'SUV', 'Van', 'Bus', 'Luxury'].map((v) => ChoiceChip(
          ...
          SwitchListTile(
            ...
            subtitle: const Text('Add family members or group passengers (one row per person)'),
            value: _isGroupBooking,
            onChanged: (v) => setState(() => _isGroupBooking = v),
          ),
          if (_isGroupBooking) ...[
            TextField(controller: _groupController, decoration: const InputDecoration(labelText: 'Group Name', prefixIcon: Icon(Icons.groups))),
            const SizedBox(height: 12),
            Text('Passengers & Family Members', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Text('Add one row per passenger. These are stored as passenger rows in the booking.', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
            const SizedBox(height: 8),
            ..._passengerRows.asMap().entries.map((entry) => _PassengerRowInput(
          ...
          SegmentedButton<String>(
            segments: const [
              ButtonSegment(value: 'flat', label: Text('Flat Rate'), icon: Icon(Icons.attach_money)),
              ButtonSegment(value: 'per_passenger', label: Text('Per Passenger'), icon: Icon(Icons.people)),
            ],
            selected: {_fareMode},
            onSelectionChanged: (v) => setState(() { _fareMode = v.first; _refreshFare(); }),
          ),
          ...
          Text(_fareMode == 'flat'
              ...
              style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
          const SizedBox(height: 12),
          TextField(controller: _priceController, keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'Your Offer (﷼)', prefixIcon: Icon(Icons.attach_money))),
          const SizedBox(height: 10),
          Card(color: AppColors.primary.withAlpha(12), child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
            _FeeRow(label: 'Platform fee (5%)', value: '﷼ ${_platformFee.toStringAsFixed(2)}'),
            const Divider(),
            _FeeRow(label: 'Driver/company receives', value: '﷼ ${(_fare - _platformFee).toStringAsFixed(2)}'),
          ]))),
          if (!_isGroupBooking) ...[
            const SizedBox(height: 12),
            Row(children: [
              ...
              const Spacer(),
              IconButton(icon: const Icon(Icons.remove_circle, color: AppColors.primary), onPressed: _passengers > 1 ? () => setState(() { _passengers--; _refreshFare(); }) : null),
              Text('$_passengers', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              IconButton(icon: const Icon(Icons.add_circle, color: AppColors.primary), onPressed: _passengers < 10 ? () => setState(() { _passengers++; _refreshFare(); }) : null),
            ]),
          ],
          const SizedBox(height: 12),
          TextField(controller: _notesController, maxLines: 2, decoration: const InputDecoration(labelText: 'Notes (optional)', prefixIcon: Icon(Icons.note))),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(
          child: TextField(
            ...
            decoration: InputDecoration(labelText: 'Passenger ${index + 1} Name', prefixIcon: const Icon(Icons.person, size: 20)),
          ),
        ),
        ...
        Expanded(
          child: TextField(
            ...
            decoration: const InputDecoration(labelText: 'Mobile (optional)', prefixIcon: Icon(Icons.phone, size: 20)),
          ),
        ),
